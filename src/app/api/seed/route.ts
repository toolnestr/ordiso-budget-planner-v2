...
// Default category templates (shared between new users and the demo seed).
...
// POST seeds: creates an admin + demo user and gives the demo user a full
// 6-month sample dataset. This endpoint is open (no auth) so first-run setup
// works, but it wipes everything — intended for evaluation only.
export async function POST() {
  try {
    for (const c of COLLECTIONS) await deleteAll(c)
    ...
    // ---- Admin gets default categories only (empty budget) ----
    await batchCreate('categories', CAT_DEFS.map((c) => ({ ...c, isSystem: true, userId: admin.id })))

    // ---- Demo user gets the full sample dataset ----
    const userId = demo.id

    // Settings (per-user doc id)
    await createWithId<Settings & { userId: string }>('settings', `singleton_${userId}`, {
      id: `singleton_${userId}`,
      userId,
      currencySymbol: '$',
      currencyCode: 'USD',
    ...
    for (const a of accountDefs) {
      const created = await createAuto<Account>('accounts', { ...a, userId, createdAt: now.toISOString() })
    ...
    for (const c of CAT_DEFS) {
      const created = await createAuto<Category>('categories', { ...c, isSystem: true, userId })
    ...
    for (const d of debtDefs) {
      const created = await createAuto<Debt>('debts', { ...d, paidOff: false, userId, createdAt: now.toISOString() })
      for (let m = 5; m >= 0; m--) {
        const date = new Date(curYear, curMonth - 1 - m, 15)
        debtPayments.push({ debtId: created.id, date: date.toISOString(), amount: d.minimumPayment, note: 'Minimum payment', userId, createdAt: now.toISOString() })
      }
    }
    ...
    for (let m = 5; m >= 0; m--) {
      ...
      txData.push({ date: mk(1), description: 'Salary Deposit', amount: 2250, type: 'INCOME', categoryId: cMap['Primary Income'], accountId: aMap['Everyday Checking'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
      txData.push({ date: mk(15), description: 'Salary Deposit', amount: 2250, type: 'INCOME', categoryId: cMap['Primary Income'], accountId: aMap['Everyday Checking'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
      if (m % 2 === 0) txData.push({ date: mk(20), description: 'Freelance — Logo Design', amount: 450 + m * 50, type: 'INCOME', categoryId: cMap['Freelance'], accountId: aMap['Everyday Checking'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
      if (m === 1 || m === 4) txData.push({ date: mk(22), description: 'Freelance — Website', amount: 900, type: 'INCOME', categoryId: cMap['Freelance'], accountId: aMap['Everyday Checking'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
      txData.push({ date: mk(28), description: 'Dividend Payout', amount: 120, type: 'INCOME', categoryId: cMap['Freelance'], accountId: aMap['High-Yield Savings'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
      txData.push({ date: mk(1), description: 'Rent Payment', amount: 1450, type: 'EXPENSE', categoryId: cMap['Rent / Housing'], accountId: aMap['Everyday Checking'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
      txData.push({ date: mk(18), description: 'Electric & Gas', amount: 120 + Math.round(Math.random() * 40), type: 'EXPENSE', categoryId: cMap['Utilities'], accountId: aMap['Everyday Checking'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
      txData.push({ date: mk(5), description: 'Internet Bill', amount: 65, type: 'EXPENSE', categoryId: cMap['Internet'], accountId: aMap['Everyday Checking'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
      txData.push({ date: mk(22), description: 'Phone Bill', amount: 55, type: 'EXPENSE', categoryId: cMap['Phone Bill'], accountId: aMap['Everyday Checking'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
      txData.push({ date: mk(12), description: 'Netflix', amount: 15.49, type: 'EXPENSE', categoryId: cMap['Entertainment'], accountId: aMap['Rewards Credit Card'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
      txData.push({ date: mk(9), description: 'Spotify', amount: 16.99, type: 'EXPENSE', categoryId: cMap['Entertainment'], accountId: aMap['Rewards Credit Card'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
      txData.push({ date: mk(3), description: 'Gym Membership', amount: 39.99, type: 'EXPENSE', categoryId: cMap['Personal Care'], accountId: aMap['Everyday Checking'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
      ...
      for (let g = 0; g < groceryRuns; g++) {
        ...
        txData.push({ date: mk(day), description: ['Whole Foods', 'Trader Joes', 'Costco', 'Local Market'][g % 4], amount: 60 + Math.round(Math.random() * 90), type: 'EXPENSE', categoryId: cMap['Groceries'], accountId: aMap['Rewards Credit Card'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
      }
      const eats = 3 + Math.floor(Math.random() * 3)
      for (let e = 0; e < eats; e++) {
        const day = Math.min(dim, 2 + e * 9 + Math.floor(Math.random() * 4))
        txData.push({ date: mk(day), description: ['Starbucks', 'Chipotle', 'Local Bistro', 'DoorDash', 'Pizza Place'][e % 5], amount: 12 + Math.round(Math.random() * 45), type: 'EXPENSE', categoryId: cMap['Dining Out'], accountId: aMap['Rewards Credit Card'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
      }
      txData.push({ date: mk(10), description: 'Shell Gas', amount: 45 + Math.round(Math.random() * 25), type: 'EXPENSE', categoryId: cMap['Transport'], accountId: aMap['Everyday Checking'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
      txData.push({ date: mk(24), description: 'Shell Gas', amount: 45 + Math.round(Math.random() * 25), type: 'EXPENSE', categoryId: cMap['Transport'], accountId: aMap['Everyday Checking'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
      if (m === 0 || m === 2) txData.push({ date: mk(14), description: 'Target Run', amount: 80 + Math.round(Math.random() * 60), type: 'EXPENSE', categoryId: cMap['Shopping'], accountId: aMap['Rewards Credit Card'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
      txData.push({ date: mk(16), description: 'Movie Tickets', amount: 28, type: 'EXPENSE', categoryId: cMap['Entertainment'], accountId: aMap['Everyday Checking'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
      if (m === 1 || m === 4) txData.push({ date: mk(12), description: 'Pharmacy — Prescription', amount: 35, type: 'EXPENSE', categoryId: cMap['Medical / Health'], accountId: aMap['Everyday Checking'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
      txData.push({ date: mk(8), description: 'Pet Food & Supplies', amount: 55, type: 'EXPENSE', categoryId: cMap['Pets'], accountId: aMap['Rewards Credit Card'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
      txData.push({ date: mk(19), description: 'Haircut', amount: 35, type: 'EXPENSE', categoryId: cMap['Personal Care'], accountId: aMap['Everyday Checking'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
      txData.push({ date: mk(2), description: 'Emergency Fund Contribution', amount: 400, type: 'EXPENSE', categoryId: cMap['Emergency Fund'], accountId: aMap['High-Yield Savings'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
      txData.push({ date: mk(2), description: 'Vacation Fund Contribution', amount: 200, type: 'EXPENSE', categoryId: cMap['Vacation Fund'], accountId: aMap['High-Yield Savings'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
      txData.push({ date: mk(2), description: 'Christmas Fund Contribution', amount: 100, type: 'EXPENSE', categoryId: cMap['Christmas Gifts'], accountId: aMap['High-Yield Savings'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
      txData.push({ date: mk(15), description: 'Visa Card Payment', amount: 250, type: 'EXPENSE', categoryId: cMap['Credit Card Debt'], accountId: aMap['Everyday Checking'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
      txData.push({ date: mk(15), description: 'Student Loan Payment', amount: 180, type: 'EXPENSE', categoryId: cMap['Student Loan'], accountId: aMap['Everyday Checking'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
      txData.push({ date: mk(15), description: 'Auto Loan Payment', amount: 320, type: 'EXPENSE', categoryId: cMap['Car Loan'], accountId: aMap['Everyday Checking'], isSplit: false, parentTransactionId: null, isReconciled: false, userId, createdAt: now.toISOString() })
    }
    ...
    const budgetDocs = Object.entries(budgetPlan).map(([name, planned]) => ({
      _id: `${userId}_${curYear}_${curMonth}_${cMap[name]}`,
      userId,
      month: curMonth,
      year: curYear,
...
export async function GET() {
  const count = await countAll('users')
  return ok({ seeded: count > 0, userCount: count })
}
